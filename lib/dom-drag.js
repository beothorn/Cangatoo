...
function addWindowFrameAndSetPosition(id,x,y){
	...
	$("#"+id).wrap('<div id="'+wrapperId+'" style="left:'+x+'px; top:'+y+'px;position:absolute;border-bottom-color: #000000;border-style: outset;border-width=1px;" />');
	$("#"+wrapperId).prepend(
		'<div id="'+titleBarId+'" style="background:#eeeeee;float:right;width:100%; text-align: right;"></div>'
	);
	
	var jqTitleBar = $("#"+titleBarId); 
		
	jqTitleBar.append(id);
	jqTitleBar.append('<input id="'+id+hideButtonSuffix+'" type="button" value="_" onClick="toogleVisibility(\''+id+'\')"/>');
	jqTitleBar.append('<input id="'+id+closeButtonSuffix+'" type="button" value="X" onClick="closeWindow(\''+wrapperId+'\')"/>');
